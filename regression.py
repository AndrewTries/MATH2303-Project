...
# binary_columns = ['Donor_Medical_Approval', 'Match_Status', 'Organ_Condition_Alert']
# for bc in binary_columns:
#     for i in bc:
#         df_clean[bc].append(i.replace(i, str(1 if i == 'Yes' else 0)))
...
for i in df['Organ_Condition_Alert']:
    Organ_Condition_Alert.append(i.replace(i, str(1 if i == 'Normal' else 0)))
...
df_clean.to_csv("../Final Project/cleaned_data.csv", index=False)
